...
def strength_along_z(in_file, start_z, end_z, steps, radius, along_x, edge_z, n_poles):
    """
    ...
    :param n_poles: The number of poles of the magnet strength you want to measure
    ...
    for z in np.linspace(start_z, end_z, steps):
        ...
        r = np.array(r)
        By = np.array(By)
        poly_fit = poly.polyfit(r, By, degree, full=True)
        poly_function = poly.polyval(r, poly_fit[0])
        # Calculate R^2
        z_to_r_squared[z] = 1. - (poly_fit[1][0][0] / sum(poly_function ** 2))
        # Calculate strength, assuming a 10 KeV electron.
        z_to_strength[z] = poly.polyder(poly_fit[0], degree)[0] / 0.00033887

    # Plot the strength along z.
    plot_strength(z_to_strength, degree, start_z, end_z, edge_z, radius, along_x)
    ...
    print("Integral: " + str(
        (sum(z_to_strength.values()) * (end_z - start_z) / len(z_to_strength.values()))) + " Tesla * meters^" + str(
        -degree + 1))
    print("Min: " + str(min(z_to_strength.values()) * 0.00033887) + " Tesla * meters^" + str(-degree))
    print("Max: " + str(max(z_to_strength.values()) * 0.00033887) + " Tesla * meters^" + str(-degree))
    print("Min: " + str(min(z_to_strength.values())) + " meters^" + str(-degree - 1))
    print("Max: " + str(max(z_to_strength.values())) + " meters^" + str(-degree - 1))
    return z_to_strength


def plot_strength(z_to_strength, degree, start_z, end_z, edge_z, radius, along_x):
    prefix = ""
    if degree == 0:
        prefix = "di"
    elif degree == 1:
    ...
    plt.xlabel("Z (meters)")
    plt.ylabel(prefix.capitalize() + "pole Strength ($m^{" + str(-degree - 1) + "}$)")
    if along_x:
        plt.title(prefix.capitalize() + "pole Strength Along x With Radius " + str(radius))
    else:
        plt.title(prefix.capitalize() + "pole Strength Along y With Radius " + str(radius))
    plt.show()
